    ...
    useEffect(() => {
        const fetchReportsData = async () => {
            try {
                ...
                const response = await fetch(`http://${process.env.NEXT_PUBLIC_LOCAL_IP}/v1/admin/reports`, {
                    headers: {
...
const useDeleteReport = (reportId: string) => {
    ...
    const deleteReport = async () => {
        ...
        try {
            ...
            const response = await fetch(`http://${process.env.NEXT_PUBLIC_LOCAL_IP}/v1/admin/deleteReport/${reportId}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
