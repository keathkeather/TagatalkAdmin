    ...
    useEffect(() => {
        const fetchFeedbackData = async () => {
            try {
                ...
                const response = await fetch(`http://${process.env.NEXT_PUBLIC_LOCAL_IP}/v1/admin/feedbacks`, {
                    headers: {
...
const useDeleteFeedback = (feedbackId: string) => {
    ...
    const deleteFeedback = async () => {
        ...
        try {
            ...
            const response = await fetch(`http://${process.env.NEXT_PUBLIC_LOCAL_IP}/v1/admin/deleteFeedback/${feedbackId}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
